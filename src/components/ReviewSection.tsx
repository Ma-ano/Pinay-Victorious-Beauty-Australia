            ...
            <AnimatePresence mode="wait">
              ...
              >
                {slides[current].map((review, i) => (
                  <div
                    key={i}
                    className={`bg-card rounded-2xl border-2 border-dashed border-accent/25 p-6 flex flex-col transition-shadow hover:shadow-lg hover:shadow-accent/5 ${rotations[i % rotations.length]}`}
                  >
                    {/* Stars */}
                    <div className="mb-3">
                    ...
                    <div className="mt-4 pt-4 border-t border-dashed border-accent/20">
                      <div className="flex items-center gap-2">
                        {review.photoURL ? (
                          <img src={review.photoURL} alt="" className="w-7 h-7 rounded-full object-cover shrink-0" />
                        ) : (
